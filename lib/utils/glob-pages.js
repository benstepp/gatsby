...
function createCache (directory, callback) {
  console.log(`[gatsby] ${'Globbing /pages'.grey}`)
  const pagesData = []
...
function childHandle (callback) {
  process.on('message', message => {
    if (message.type === 'GLOB_RESPONSE') {
      callback(null, message.payload)
    }
  })
  process.send({ type: 'GLOB_REQUEST' })
}

function parentHandle (directory, callback) {
...
module.exports = function globPages (directory, callback) {
  if (typeof process.send === 'undefined') {
    parentHandle(directory, callback)
  } else {
    childHandle(callback)
  }
}
